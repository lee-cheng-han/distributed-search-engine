...
std::expected<void,WriterError> IndexWriter::compact_published(){
 ...
 SegmentId id(1);{std::lock_guard lock(mutex_);id=SegmentId(next_segment_id_++);}auto merged=SegmentMerger::merge_segments(selected,directory_,id,options_.compressed_segments);if(!merged)return std::unexpected(failure(WriterErrorCode::storage_error,merged.error().message));
