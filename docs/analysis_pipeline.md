...
The `StandardAnalyzer` scans validated `std::string_view` input without copying it first. ASCII
...
The `KeywordAnalyzer` emits a non-empty value as one verbatim token at position zero. It is useful
for exact-match fields such as tags. Empty values produce no tokens.
